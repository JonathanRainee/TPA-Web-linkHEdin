...
import { AddMessage, AddMessageSharePost, AddRoom, UserConnected } from '../queries/queryUser';

const ModalShare = ({ postData, roomData, userBlockData, closeModal }: any) => {
    ...
    const [sharePostMutation] = useMutation(AddMessageSharePost)
    ...
    useEffect(()=>{
        for (let index = 0; index < roomData.length; index++) {
            ...
            console.log(roomData[index].id)
            console.log(selectedUser.userId)
            if(roomData[index].user2.id === selectedUser.userId || roomData[index].user1.id === selectedUser.userId){
                // console.log("asdfasd");
                console.log(roomData[index].id)
                setRoomId(roomData[index].id)
                return
            }
        }
    }, [selectedUser.userId])
    ...
    const handleSharePost = (roomId: string) => {
        console.log("aidi "+postData.id)
        sharePostMutation({
            variables:{
                senderId: UserContext.user.id,
                roomId: roomId,
